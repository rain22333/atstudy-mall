<template>
	<div class="container box-shadow rounded py-2 px-4 border-box">
		...
		<div class="d-flex mx-2 text-muted te-xs" v-if="product.selected_category == undefined">
			...
			<div class='mx-2 hand' v-for="cate of product.category_list" :key=" 'cate' + cate.cate_id "
				@click="cateClicked(cate)" :class="{'text-red' : product.selected_category == cate}">{{cate.cate_name}}
			</div>
		</div>
		<!-- /分类信息 -->
		...
		<div class="d-flex text-muted te-xs mt-2 " v-for="(attr,index) of product.attr_list"
			...
			<div class="d-flex flex-wrap">
				<div class='mx-2 p-1 hand' v-for=" key_value of attr.spuAttrValueList " :key=" 'value' + key_value.id "
					@click="attr_clicked({'index' : index,'attr' : key_value})"
					:class="{'text-red' : product.selected_attr_list[index] == key_value}">{{key_value.value_name}}
				</div>
			</div>
		</div>
	</div>
